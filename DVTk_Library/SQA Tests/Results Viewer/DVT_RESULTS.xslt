...
<!-- edited with XMLSPY v2004 rel. 3 U (http://www.xmlspy.com) by Piyush (Philips Medical Systems Ned. B.V.) -->
<?xmlspysamplexml C:\Program Files\DVT\example\media\results\Detail_001_DICOMDIR_1_res.xml?>
		...
		<html>
			...
			<body>
				...
				<xsl:for-each select="//MessageUID">
					...
					<xsl:element name="a">
						<xsl:attribute name="href"><xsl:value-of select="substring-before(//DvtSummaryResultsFilename,'Summary')"/>Detail<xsl:value-of select="substring-before(substring-after(//DvtSummaryResultsFilename,'Summary'),'.xml')"/>.html#UID<xsl:value-of select="."/></xsl:attribute>
		...
		<xsl:for-each select="ValidationCounters">
			<xsl:for-each select="document($filterXMLfilename)/Nodes/added/Message">
				<xsl:if test="substring-before(Name,'-')=$filename">
					<br/>
					<xsl:element name="a">
						<xsl:attribute name="href">ADD-<xsl:value-of select="Name"/></xsl:attribute>
						<font color="#FF0000">
							<br/>Manualy added Error: <xsl:value-of select="substring-after(Name,'-')"/>
							<br/>
						</font>
					</xsl:element>(<xsl:element name="a">
						<xsl:attribute name="href">DEL-<xsl:value-of select="Name"/></xsl:attribute>Click here to delete error.</xsl:element>)<br/>
				</xsl:if>
			</xsl:for-each>
			<br/>
				...
				<xsl:choose>
					<xsl:when test="$messagetype='HTMLUserActivity'">
						<xsl:choose>
							...
							<xsl:otherwise>
								<xsl:element name="a">
									<xsl:attribute name="href"><xsl:value-of select="$index"/> - <xsl:value-of select="$filename"/></xsl:attribute>
									<font color="#FF0000">
										<xsl:value-of select="$type"/>: 
     <xsl:call-template name="converttohtml">
											<xsl:with-param name="text">
