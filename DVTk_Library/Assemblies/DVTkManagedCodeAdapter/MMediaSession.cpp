...
namespace Wrappers
{
    ...
    void MMediaSession::DisposeThis() {
        DisposeThis(true);
    ...
    void MMediaSession::DisposeThis(bool disposing) {
        ...
        if (!this->disposed) {
            ...
            m_pMEDIA_SESSION_CLASS = nullptr;
        }
        disposed = true;
    }

  	System::Boolean MMediaSession::GenerateDICOMDIR(cli::array<System::String^>^ dcmFileNames)
    {
    ...
    {
        bool success = false;
        if (pDicomFile == nullptr) throw gcnew System::ArgumentNullException();
        VALIDATION_CONTROL_FLAG_ENUM umValidationControlFlags = _Convert(validationControlFlags);
        FILE_DATASET_CLASS* fileDataset_ptr = ManagedUnManagedMediaConvertors::ManagedUnManagedMediaConvertor::Convert(pDicomFile, nullptr);
