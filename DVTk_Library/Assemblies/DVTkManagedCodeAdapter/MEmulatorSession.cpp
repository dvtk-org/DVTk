...
namespace Wrappers
{
    MEmulatorSession::MEmulatorSession(void)
    {
        ...
        m_pPrinter = gcnew Wrappers::MPrinter();
        disposed = false;
        this->Initialize();
        return;
    }
    ...
    void MEmulatorSession::DisposeThis() {
        DisposeThis(true);
    ...
    void MEmulatorSession::DisposeThis(bool disposing) {
        ...
        if (!this->disposed) {
			...
			if(m_pEMULATOR_SESSION_CLASS != nullptr)
			{
				delete m_pEMULATOR_SESSION_CLASS;
				m_pEMULATOR_SESSION_CLASS = nullptr;
			}
        }
        disposed = true;
    }

    MPrinter^ MEmulatorSession::MPrinter::get()
    {
        return m_pPrinter;
    }
        ...
        System::UInt32 repetitions)
    {
        ...
        // Therefor no heap-allocation with gcnew is needed for vector<string> fileNameVector
