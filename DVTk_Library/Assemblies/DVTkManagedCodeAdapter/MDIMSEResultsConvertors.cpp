...
namespace ManagedUnManagedDimseValidationResultsConvertors
{
        ...
        VAL_OBJECT_RESULTS_CLASS *pUMValidationObjectResult, UINT flags)
    {
        ...
        // Preconditions
        //
        if (pUMValidationObjectResult == nullptr) return nullptr;
        //
        // Create node
        //
        DvtkData::Validation::ValidationObjectResult ^ pValidationObjectResult =
            gcnew DvtkData::Validation::ValidationObjectResult();
        //
        // Convert parameters
        //
        System::String^ clistr = gcnew System::String(ConvertString(pUMValidationObjectResult->GetName()).c_str());
        pValidationObjectResult->Name = clistr;
        clistr = gcnew System::String(ConvertString(pUMValidationObjectResult->GetDICOMDIRName()).c_str());
		pValidationObjectResult->DICOMDIRName = clistr;
        clistr = gcnew System::String(ConvertString(pUMValidationObjectResult->GetSpecificCharacterSetValues()).c_str());
        pValidationObjectResult->SpecificCharacterSetValues = clistr;
        for (UINT i = 0; i < (UINT)pUMValidationObjectResult->GetNrModuleResults(); i++)
        {
			...
			{
				DvtkData::Validation::SubItems::ValidationAttributeGroupResult ^ pValidationAttributeGroupResult = Convert(module_ptr, flags);
        ...
        if (pUMValidationObjectResult->HasMessages())
        {
            ...
            {
                DvtkData::Validation::ValidationMessage^ pValidationMessage =
                    gcnew DvtkData::Validation::ValidationMessage();
                ...
                clistr = gcnew System::String(ConvertString(pUMLOG_MESSAGE_CLASS->GetMessage(i)).c_str());
        ...
        //
        if (
            pValidationObjectResult->AdditionalAttributes != nullptr &&
        ...
        RECORD_LINK_VECTOR* pUMRecordLinkVector)
    {
        ...
        DvtkData::Validation::TypeSafeCollections::ValidationDirectoryRecordLinkCollection^
            ...
            = gcnew DvtkData::Validation::TypeSafeCollections::ValidationDirectoryRecordLinkCollection();
        //
        // Convert individual record links
        //
        for (UINT dirRecIndex = 0; dirRecIndex < pUMRecordLinkVector->size(); dirRecIndex++)
        {
            DvtkData::Validation::SubItems::ValidationDirectoryRecordLink^
                pValidationDirectoryRecordLink =
                gcnew DvtkData::Validation::SubItems::ValidationDirectoryRecordLink();
            ...
            if (pRECORD_LINK->HasMessages())
            {
                ...
                for (UINT i = 0; i < (UINT)pUMLOG_MESSAGE_CLASS->GetNrMessages(); i++)
                {
                    ...
                    System::UInt32 messageUID = pUMLOG_MESSAGE_CLASS->GetMessageId(i);
                    pValidationMessage->Identifier = messageUID;
                    System::String^ clistr = gcnew System::String(ConvertString(pUMLOG_MESSAGE_CLASS->GetMessage(i)).c_str());                    
                    pValidationMessage->Message = clistr;
        ...
        RECORD_RESULTS_CLASS *pUMValidationDirectoryRecordResult, UINT flags)
    {
        ...
        // Preconditions
        //
        if (pUMValidationDirectoryRecordResult == nullptr) return nullptr;
        ...
        //
        interior_ptr<ManagedUnManagedDimseValidationResultsConvertor> pV1 = this;
		/*System::UInt32 oldNrOfErrors = this->m_pCountingTarget->NrOfErrors->get();
		System::UInt32 oldNrOfWarnings = this->m_pCountingTarget->NrOfWarnings->get();*/
        System::UInt32 oldNrOfErrors = this->m_pCountingTarget->NrOfErrors;
        System::UInt32 oldNrOfWarnings = this->m_pCountingTarget->NrOfWarnings;
        //
        // Create node
        //
        DvtkData::Validation::ValidationDirectoryRecordResult ^pValidationDirectoryRecordResult = gcnew DvtkData::Validation::ValidationDirectoryRecordResult();
        //
        // Convert parameters
        //
        for (UINT i = 0; i < (UINT)pUMValidationDirectoryRecordResult->GetNrAttributes(); i++)
        {
            DvtkData::Validation::SubItems::ValidationAttributeResult ^ pValidationAttributeResult =
        ...
        if (pUMValidationDirectoryRecordResult->GetDefAttributeGroup())
        {
            ...
            {
                System::String^ clistr = gcnew System::String(ConvertString(pUMValidationDirectoryRecordResult->GetDefAttributeGroup()->GetTextualCondition()).c_str());
                pValidationDirectoryRecordResult->ConditionText = clistr;
            }
        }
        ...
        if (pUMValidationDirectoryRecordResult->GetDefAttributeGroup())
        { 
            System::String^ clistr = gcnew System::String(ConvertString(pUMValidationDirectoryRecordResult->GetDefAttributeGroup()->GetName()).c_str());
            pValidationDirectoryRecordResult->Name = clistr;
        ...
        //
		/*System::UInt32 newNrOfErrors = this->m_pCountingTarget->NrOfErrors->get();
		System::UInt32 newNrOfWarnings = this->m_pCountingTarget->NrOfWarnings->get();*/
        System::UInt32 newNrOfErrors = this->m_pCountingTarget->NrOfErrors;
        System::UInt32 newNrOfWarnings = this->m_pCountingTarget->NrOfWarnings;
        ...
        VAL_ATTRIBUTE_GROUP_CLASS *pUMValidationAttributeGroupResult, UINT flags)
    {
        ...
        // Preconditions
        //
        if (pUMValidationAttributeGroupResult == nullptr) return nullptr;
        //
        // Create node
        //
        DvtkData::Validation::SubItems::ValidationAttributeGroupResult ^ pValidationAttributeGroupResult =
            gcnew DvtkData::Validation::SubItems::ValidationAttributeGroupResult();
        //
        // Convert parameters
        //
        ...
        if (pUMValidationAttributeGroupResult->GetDefAttributeGroup())
        {
            ...
            {
                System::String^ clistr = gcnew System::String(ConvertString(pUMValidationAttributeGroupResult->GetDefAttributeGroup()->GetTextualCondition()).c_str());
                pValidationAttributeGroupResult->ConditionText = clistr;
            }
        }
        ...
        if (pUMValidationAttributeGroupResult->GetDefAttributeGroup())
        { 
            System::String^ clistr = gcnew System::String(ConvertString(pUMValidationAttributeGroupResult->GetDefAttributeGroup()->GetName()).c_str());
            System::String^ clistr_value_condition = gcnew System::String(ConvertString(pUMValidationAttributeGroupResult->GetDefAttributeGroup()->GetValueConditionText()).c_str());
            pValidationAttributeGroupResult->Name = clistr;
            pValidationAttributeGroupResult->ValueConditionText = clistr_value_condition;
        ...
        VAL_ATTRIBUTE_CLASS *pUMValidationAttributeResult, UINT flags)
    {
        ...
        // Preconditions
        //
        if (pUMValidationAttributeResult == nullptr) return nullptr;
        ...
        if (pAttr == nullptr) pAttr = dcm_attr;
        if (pAttr == nullptr) pAttr = ref_attr;
        if (pAttr == nullptr) System::Diagnostics::Trace::Assert(false);
        ...
        DvtkData::Validation::SubItems::ValidationAttributeResult
            ^ pValidationAttributeResult =
            gcnew DvtkData::Validation::SubItems::ValidationAttributeResult();
        ...
        //
        pValidationAttributeResult->Presence = (dcm_attr != nullptr);
        ...
        System::String^ clistr = gcnew System::String(ConvertString(attributeName).c_str());
        pValidationAttributeResult->Name = clistr;
        ...
        pValidationAttributeResult->Tag = gcnew DvtkData::Dimse::Tag(group, element);
        ...
        UINT flags)
    {
        ...
        DvtkData::Validation::SubItems::ValidationValueResult^
            pValidationValueResult =
            gcnew DvtkData::Validation::SubItems::ValidationValueResult();
        //
        // Convert parameters
        //
        ...
        //
        System::Object^ item = nullptr;
        if (vr == ::ATTR_VR_SQ)
        {
            VAL_VALUE_SQ_CLASS * sq = nullptr;
#ifdef NDEBUG
            ...
            auto sequenceOfItems = gcnew cli::array<DvtkData::Validation::SubItems::ValidationAttributeGroupResult^>(nrOfSequenceItems);
		...
		else if (vr == ::ATTR_VR_US)
		{
            if (pUMValBaseValue->GetDcmValue())
            {
				...
				item = gcnew System::String(buffer);
			}
		}
        ...
        else
        {
            ...
            if (pUMValBaseValue->GetDcmValue())
            {
                ...
                item = gcnew System::String(valueString.c_str());
           }
        }
    ...
    // </remarks>
    void ManagedUnManagedDimseValidationResultsConvertor::set_Rules(System::Uri^ pRulesUri)
    {
    ...
    // </remarks>
	void ManagedUnManagedDimseValidationResultsConvertor::set_CountingTarget(ICountingTarget^ pCountingTarget)
    {
