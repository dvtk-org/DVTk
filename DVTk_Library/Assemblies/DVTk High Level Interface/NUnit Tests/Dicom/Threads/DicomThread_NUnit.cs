    ...
    public class DicomThread_NUnit
    {
        ...
        public class CFINDDicomThread : DicomThread
        {
            ...
            protected override void Execute()
            {
                ...
                scp.Options.ResultsFileNameOnlyWithoutExtension = resultFilename;
                scp.Options.LoadFromFile(Path.Combine(Paths.SessionDirectoryFullPath, "1266_CFind_SCP.ses"));
       ...
       /// </summary>        
        ...
        private SecureScu secureScu = null;

        private SecureScp secureScp = null;
        ...
        private class Secure1358DicomThread : DicomThread
        {
            ...
            protected override void Execute()
            {
                ...
                Ticket1358scu.Options.DvtkScriptSession.SecuritySettings.MaxTlsVersionFlags = DvtkSession.TlsVersionFlags.TLS_VERSION_TLSv1_0;
                Ticket1358scu.Options.DvtkScriptSession.SecuritySettings.MinTlsVersionFlags = DvtkSession.TlsVersionFlags.TLS_VERSION_TLSv1_0;
                Ticket1358scp.Options.DvtkScriptSession.SecuritySettings.MaxTlsVersionFlags = DvtkSession.TlsVersionFlags.TLS_VERSION_TLSv1_0;
                Ticket1358scp.Options.DvtkScriptSession.SecuritySettings.MinTlsVersionFlags = DvtkSession.TlsVersionFlags.TLS_VERSION_TLSv1_0;
        ...
        private class SecureScu : DicomThread
        {
            private int milliSecondsToWaitForPendingDataInNetworkInputBuffer = 0;

            private bool hasPendingDataInNetworkInputBuffer = true;

            public SecureScu(int milliSecondsToWaitForPendingDataInNetworkInputBuffer)
            {
                this.milliSecondsToWaitForPendingDataInNetworkInputBuffer = milliSecondsToWaitForPendingDataInNetworkInputBuffer;
            }
        ...
        private class SecureScp : DicomThread
        {
            private int milliSecondsToWaitForPendingDataInNetworkInputBuffer = 0;

            private bool hasPendingDataInNetworkInputBuffer = true;

            public SecureScp(int milliSecondsToWaitForPendingDataInNetworkInputBuffer)
            {
                this.milliSecondsToWaitForPendingDataInNetworkInputBuffer = milliSecondsToWaitForPendingDataInNetworkInputBuffer;
            }
        ...
        private void secureConnection(int timeoutScu, int timeoutScp)
        {
            ThreadManager threadManager = new ThreadManager();

            this.secureScu = new SecureScu(timeoutScu);
            secureScu.Initialize(threadManager);

            this.secureScp = new SecureScp(timeoutScp);
            secureScp.Initialize(threadManager);

            secureScu.Options.LoadFromFile(Path.Combine(Paths.SessionDirectoryFullPath, "secure_connection_scu.ses"));
            secureScp.Options.LoadFromFile(Path.Combine(Paths.SessionDirectoryFullPath, "secure_connection_scp.ses"));

            secureScu.Options.ResultsFileNameOnlyWithoutExtension = "secure_scu_" + timeoutScu.ToString();
            secureScu.Options.DvtkScriptSession.SecuritySettings.MaxTlsVersionFlags = DvtkSession.TlsVersionFlags.TLS_VERSION_TLSv1_0;
            secureScu.Options.DvtkScriptSession.SecuritySettings.MinTlsVersionFlags = DvtkSession.TlsVersionFlags.TLS_VERSION_TLSv1_0;

            secureScp.Options.ResultsFileNameOnlyWithoutExtension = "secure_scp_" + timeoutScp.ToString();
            secureScp.Options.DvtkScriptSession.SecuritySettings.MaxTlsVersionFlags = DvtkSession.TlsVersionFlags.TLS_VERSION_TLSv1_0;
            secureScp.Options.DvtkScriptSession.SecuritySettings.MinTlsVersionFlags = DvtkSession.TlsVersionFlags.TLS_VERSION_TLSv1_0;

            secureScp.Start();
            secureScu.Start(100);

            secureScp.WaitForCompletion();
            secureScu.WaitForCompletion();
        }

        [Test]
        public void secure_connection_1()
        {
            secureConnection(2000, 4000);
            Assert.That(secureScu.HasExceptionOccured, Is.EqualTo(false));
        }

        [Test]
        public void secure_connection_2()
        {
            secureConnection(2000, 4000);
            Assert.That(secureScp.HasExceptionOccured, Is.EqualTo(false));
        }
    }
}
