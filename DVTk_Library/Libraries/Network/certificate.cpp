...
//<<===========================================================================
{
	...
	if (publicKey_ptr != NULL)
	{
		...
		keyType = EVP_PKEY_id(publicKey_ptr);
		
		// prev. publicKey_ptr->type == EVP_PKEY_RSA
		if (keyType == EVP_PKEY_RSA)
		{
			signatureAlgorithmM = "RSA";
			keySize = EVP_PKEY_bits(publicKey_ptr);
			//prev. if ((publicKey_ptr->pkey.rsa != NULL) && (publicKey_ptr->pkey.rsa->n != NULL))
			if (keySize > 0)
			{
				signatureKeyLengthM = keySize;
			}
			else
			{
		...
		else if (keyType == EVP_PKEY_DSA)
		{
			...
			keySize = EVP_PKEY_bits(publicKey_ptr);
			//prev. if ((publicKey_ptr->pkey.dsa != NULL) && (publicKey_ptr->pkey.dsa->p != NULL))
			if (keySize > 0)
			{
				signatureKeyLengthM = keySize;
			} 
...
//<<===========================================================================
{
	...
	signatureKeyLengthM = 0;
	signatureAlgorithmM = "";
	keyType = EVP_PKEY_id(key_ptr);
	if (keyType == EVP_PKEY_RSA) //prev. if(key_ptr->type == EVP_PKEY_RSA)
	{
		signatureAlgorithmM = "RSA";
		subjectM = "RSA Private Key";
		keySize = EVP_PKEY_bits(key_ptr);
		if (keySize > 0) // prev. if ((key_ptr->pkey.rsa != NULL) && (key_ptr->pkey.rsa->n != NULL))
		{
			signatureKeyLengthM = keySize; // prev. signatureKeyLengthM = BN_num_bits(key_ptr->pkey.rsa->n);
		}
		else
		{
	...
	else if (keyType == EVP_PKEY_DSA) //prev. if (key_ptr->type == EVP_PKEY_DSA)
	{
		signatureAlgorithmM = "DSA";
		subjectM = "DSA Private Key";
		keySize = EVP_PKEY_bits(key_ptr);
		if (keySize > 0) // if ((key_ptr->pkey.dsa != NULL) && (key_ptr->pkey.dsa->p != NULL))
		{
			//prev. signatureKeyLengthM = BN_num_bits(key_ptr->pkey.dsa->p);
			signatureKeyLengthM = keySize;
		}
		else
		{
...
//<<===========================================================================
{
	...
	// Release Notes Starting with ::OpenSSL 1.0.0, digest algorithms are no longer restricted, any ::OpenSSL::Digest may be used for signing.
	//Fixed digest method for all certificate types
	digest_ptr = EVP_sha256();
	/*
	...
	certBio_ptr = BIO_new(BIO_s_file());
	if (certBio_ptr == NULL)
	{
	...
	if (isCredentials)
	{
		...
		for (i = 0; i < numberOfCerts; i++)
		{
			...
			if (x509Info_ptr->x_pkey != NULL)
			{
				...
				keyType = EVP_PKEY_id(x509Info_ptr->x_pkey->dec_pkey);
				if (keyType == EVP_PKEY_RSA) // .prev if (x509Info_ptr->x_pkey->dec_pkey->type == EVP_PKEY_RSA)
				{
					// rsa key
					if (rsaKeyRead)
					{
				...
				else if (keyType == EVP_PKEY_DSA) // prev. if (x509Info_ptr->x_pkey->dec_pkey->type == EVP_PKEY_RSA)
				{
					// dsa key
					if (dsaKeyRead)
					{
