...
//<<===========================================================================
{
	...
	if (logger_ptr)
	{
		...
		{
			//logger_ptr->text(logLevel, 1, "Secure Socket - INFO %s", buffer);
		}
		else 
		{
...
//<<===========================================================================
{
	...
	// Old version to control added chipers
	//if (result)
	//{
	...
	//New version to control added chipers
	//Openssl implementation notes
	//Note: SSL_get_ciphers(), SSL_CTX_get_ciphers() and SSL_get_client_ciphers() return a pointer to an internal cipher stack, 
	//which will be freed later on when the SSL or SSL_SESSION object is freed. Therefore, the calling code MUST NOT free the return value itself.
	if (result) {
		auto chiperList = SSL_CTX_get_ciphers(ctx_ptr); // Do not free it as its stated above
		if (chiperList == NULL)
			result = false;
	}
	...
	*dsaCertChain_ptrptr = NULL;
	...
	while (PEM_read_bio(bio_ptr, &name_ptr, &header_ptr, &data_ptr, &len))
	{
		...
		if (checkPemName(name_ptr, PEM_STRING_EVP_PKEY))
		{
			...
			// prev. unsigned char *tmpData_ptr = data_ptr; // don't modify data_ptr below
			const unsigned char *tmpData_ptr = data_ptr; // don't modify data_ptr below
			EVP_PKEY *key_ptr = NULL;
...
{
	// load the library
	ssleayHandleM = LoadLibrary("libssl-3.dll");
	if (ssleayHandleM == NULL)
	{
		return false;
	}
...
{
	// load the library
	libeayHandleM = LoadLibrary("libcrypto-3.dll");
	if (libeayHandleM == NULL)
	{
		return false;
	}
