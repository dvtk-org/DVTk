...
//<<===========================================================================
{
	...
	case ATTR_VR_SV: 
		{
			// encode attribute SV value(s)
			// attribute length must be a multiple of VR length
			if ((length % SV_LENGTH) != 0) return false;
	...
	case ATTR_VR_UV: 
		{
			// encode attribute UV value(s)
			// attribute length must be a multiple of VR length
			if ((length % UV_LENGTH) != 0) return false;
			for (int i = 0; i < GetNrValues(); i++) 
			{
				// get a single value
				DCM_VALUE_UV_CLASS *value_ptr = NULL;
				UINT64 base_value;
				string identifier;
#ifdef NDEBUG
				VALUE_UV_CLASS *base_value_ptr = static_cast<VALUE_UV_CLASS*>(GetValue(i));
				if (base_value_ptr == NULL)
				{					
					return false; // cannot happen
				}
				else
				{
					...
					if(base_value != 0)
					{
						value_ptr = static_cast<DCM_VALUE_UV_CLASS*>(base_value_ptr);
						if (value_ptr == NULL)
						{					
							return false; // cannot happen
						}
...
#else
				VALUE_UV_CLASS *base_value_ptr = dynamic_cast<VALUE_UV_CLASS*>(GetValue(i));
				if (base_value_ptr == NULL)
				{					
					return false; // cannot happen
				}
				else
				{
					...
					if(base_value != 0)
					{
						value_ptr = dynamic_cast<DCM_VALUE_UV_CLASS*>(base_value_ptr);
						if (value_ptr == NULL)
						{					
							return false; // cannot happen
						}
				...
				if (identifier.length())
				{
					value = dataTransfer.getItemOffset64(identifier);
				}
				else 
				{
					value = base_value;
	...
	case ATTR_VR_UV: 
		{
			...
			while (length) 
			{
				...
				// save single attribute value
				DCM_VALUE_UV_CLASS *value_ptr = new DCM_VALUE_UV_CLASS();
				value_ptr->Set(value);
				AddValue(value_ptr);
				length -= sizeof(value);
			}
