...
//<<===========================================================================
{
	...
	for (UINT i = 0; i < definitionFileM_ptr.getSize(); i++)
	{
		//first take into account the "All Dimse Commands.def" in the definition file folder
		DEFINITION_FILE_CLASS* definitionFile_ptr = definitionFileM_ptr[i];
		string filename = definitionFile_ptr->getFilename();
		if (strcmp(filename.c_str(), "All DIMSE Commands.def") == 0) {
