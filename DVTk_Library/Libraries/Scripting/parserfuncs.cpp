	...
	case ATTR_VR_SV:
		{
			...
			{
				// VTS support - will not be documented
				// - need to convert string to SV value
				INT64 data = (INT64) atoi(stringValue_ptr);
	...
	case ATTR_VR_UV:
		{
			...
			case TAG_MRDR_DIRECTORY_RECORD_OFFSET:
				{
					// instantiate a value - used later to index the actual computed value
					DCM_VALUE_UV_CLASS *valueUV_ptr = new DCM_VALUE_UV_CLASS();
					valueUV_ptr->setIdentifier(stringValue_ptr);
					value_ptr = valueUV_ptr;
				}
				break;
			...
			default:
				{
					...
					if (length)
					{
						// VTS support - will not be documented
						// - need to convert string to UL value
						UINT64 data = (UINT64) atoi(stringValue_ptr);
						DCM_VALUE_UV_CLASS *valueUV_ptr = new DCM_VALUE_UV_CLASS();
						valueUV_ptr->Set(data);
						value_ptr = valueUV_ptr;
					}
				}
				break;
...
//<<===========================================================================
{
	...
	case ATTR_VR_UV:
		{
			DCM_VALUE_UV_CLASS *valueUV_ptr = new DCM_VALUE_UV_CLASS();
			valueUV_ptr->Set((UINT64) hexValue);
			value_ptr = valueUV_ptr;
		}
		break;
	...
	case ATTR_VR_SS:
		if (logger_ptr)
		{
			logger_ptr->text(LOG_ERROR, 1, "SL/SS/SV Value in DICOMScript should be expressed in INTEGER format for attribute (%04X,%04X)", group, element);
			logger_ptr->text(LOG_NONE, 1, "Value not used - zero length taken");
		}
		break;
...
//<<===========================================================================
{
	...
	case ATTR_VR_UV:
		{
			DCM_VALUE_UV_CLASS *valueUV_ptr = new DCM_VALUE_UV_CLASS();
			valueUV_ptr->Set((UINT64) intValue);
			value_ptr = valueUV_ptr;
		}
		break;
